<template>
  <PageLayout narrow>
    <PageHeader title="Umów wizytę" subtitle="Zaplanuj nową wizytę pacjenta">
      <template #actions>
    ...
    <form class="appt-form" @submit.prevent="handleSubmit">
      <!-- Date/time header -->
        ...
        <div class="appt-form__field">
          ...
          >
            ...
            <option v-for="d in doctors" :key="d.id" :value="d.id">
              {{ getDoctorOptionLabel(d, form.appointmentDate) }}
            </option>
          </select>
      ...
      <div class="appt-form__field">
        ...
        <input type="hidden" :value="form.visitType" required />
        <p v-if="!form.visitType" class="appt-form__field-warn">Wybierz typ wizyty</p>
      </div>
      ...
      <div class="appt-form__field">
        ...
        <textarea
          ...
          rows="3"
          placeholder="Dodatkowe informacje dla lekarza, uwagi pacjenta..."
        />
      </div>
      ...
      <div class="appt-form__actions">
        ...
        <button
          ...
          :disabled="submitting || !form.visitType || !isSelectedDoctorAvailable"
        >
          <span v-if="submitting">Dodawanie wizyty...</span>
          <span v-else>Umów wizytę →</span>
        </button>
      </div>
    </form>
  </PageLayout>
...
const form = ref<AppointmentFormPayload>(emptyAppointmentForm())
...
const isSelectedDoctorAvailable = computed(() => {
  if (!selectedDoctor.value || !form.value.appointmentDate) return true
  return isDoctorAvailable(selectedDoctor.value, form.value.appointmentDate)
})
...
onMounted(async () => {
  try {
    const [d, p] = await Promise.all([getDoctors(), getPatients()])
    doctors.value = d
    patients.value = p

    const dateParam = route.query.date as string | undefined
    const timeParam = route.query.time as string | undefined
    const doctorParam = route.query.doctorId as string | undefined
    const patientParam = route.query.patientId as string | undefined

    if (dateParam) {
      const time = timeParam ?? '09:00'
      form.value.appointmentDate = `${dateParam}T${time}`
    }
    if (doctorParam && doctors.value.find((doc) => doc.id === Number(doctorParam))) {
      form.value.doctorId = Number(doctorParam)
    }
    if (patientParam && patients.value.find((pat) => pat.id === Number(patientParam))) {
      form.value.patientId = Number(patientParam)
    }
  } catch (err) {
    error.value = getApiErrorMessage(err, 'Nie udało się załadować danych formularza.')
  }
})

const handleSubmit = async () => {
  if (!form.value.patientId || !form.value.doctorId || !form.value.visitType) {
    error.value = 'Uzupełnij wszystkie wymagane pola: pacjent, lekarz, typ wizyty.'
    return
  }
  if (selectedDoctor.value && !isDoctorAvailable(selectedDoctor.value, form.value.appointmentDate)) {
  ...
  try {
    ...
    error.value = getApiErrorMessage(err, 'Nie udało się dodać wizyty.')
  } finally {
